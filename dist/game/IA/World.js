...
export class World {
    ...
    createTile(id, layer, tilePosition) {
        if (layer.data[id] <= 0)
            return;
        const tileset = this.getTilesets(layer.data[id]);
        ...
        const name = `pos${position.x}x${position.y}_tile${tilePosition.x}x${tilePosition.y}_type${TileType.Solid}_id${tiledmapIndex}`;
    ...
    configureTile(tile, tileset) {
        ...
        if (tile.properties.solid) {
            ...
            tile.hitArea = new Rectangle(Math.round(data.width), Math.round(data.height), Math.round(data.x), Math.round(data.y));
    ...
    orderLayers(layer, tilePosition) {
        switch (layer) {
            case 'Above':
                return 999;
            case 'Grounded':
